...
- 每轮调用模型时，`Agent` 会把当前会话 id 填入底层 **`ModelParams.sessionId`**；可选 **`ModelParams.metadata`**（对象或函数）由各适配器决定是否映射到 HTTP 请求（Anthropic 会与 `sessionId` 一并合并进 Messages API 的 `metadata`）。字段说明见 [`sdk-types-reference.md`](./sdk-types-reference.md)。
