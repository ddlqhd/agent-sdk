...
/**
 * Build merged system prompt for a subagent run (type fragment + profile body + request override).
 */
export function buildSubagentMergedSystemPrompt(
  ...
  const parts = [typeFragment, profile.promptBody, requestSystemPrompt].filter(
